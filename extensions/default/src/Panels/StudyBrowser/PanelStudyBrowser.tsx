...
function _mapDisplaySets(displaySets, thumbnailImageSrcMap) {
  ...
  displaySets
    ...
    .filter(ds => !_shouldHideDisplaySet(ds))
    .forEach(ds => {
...
const thumbnailNoImageModalities = ['SR', 'SEG', 'SM', 'RTSTRUCT', 'RTPLAN', 'RTDOSE', 'DS'];

function _shouldHideDisplaySet(displaySet) {
  const modality = displaySet?.Modality?.toUpperCase?.();

  return modality === 'SR' || modality === 'DS';
}
