...
export default function PanelReport(props) {
  ...
  const NON_DIAGNOSTIC_MODALITIES = ['SR', 'DS'];
  const normalizedModality = modalityValue => modalityValue?.toUpperCase?.();
  const viewableDisplaySets = displaySets?.filter(ds => {
    const modality = normalizedModality(ds?.Modality);
    return modality ? !NON_DIAGNOSTIC_MODALITIES.includes(modality) : true;
  });

  // eslint-disable-next-line react/prop-types
  const activeDisplaySetFromViewport = displaySets.find(
    ds => ds.displaySetInstanceUID === displaySetInstanceUID
  );

  const activeDisplaySet =
    activeDisplaySetFromViewport &&
    !NON_DIAGNOSTIC_MODALITIES.includes(normalizedModality(activeDisplaySetFromViewport.Modality))
      ? activeDisplaySetFromViewport
      : viewableDisplaySets?.[0] || activeDisplaySetFromViewport;

  if (!activeDisplaySet) {
    return null;
  }

  const modality = activeDisplaySet.Modality || 'N/A';
